...
function vitePluginManusDebugCollector(): Plugin {
  return {
    ...
    transformIndexHtml(html) {
      // Injeção do script de debug apenas em dev
      return {
        html,
        tags: [
          {
      ...
      server.middlewares.use("/__manus__/logs", (req, res, next) => {
        ...
        if (reqBody && typeof reqBody === "object") {
          try { handlePayload(reqBody); } catch (e) {
            res.writeHead(400).end(JSON.stringify({ success: false, error: String(e) }));
          }
          return;
        }
...
const plugins = [
  ...
  // Debug collector só em desenvolvimento (já definido no plugin com apply: 'serve')
...
export default defineConfig({
  ...
  build: {
    ...
    emptyOutDir: true,
    // Em produção, minimizar e otimizar
    minify: 'esbuild',
    sourcemap: false,
    target: 'esnext',
    rollupOptions: {
      output: {
        manualChunks: {
          ...
          // Você pode adicionar outras bibliotecas grandes aqui
        }
      }
    }
  },
