...
app.use(helmet({
    contentSecurityPolicy: {
        directives: {
            ...
            connectSrc: ["'self'", "http://localhost:3007", "ws://localhost:3008"],
            fontSrc: ["'self'"],
        },
    },
}));

// 6. CSRF – implementação manual com 'csrf'
...
// 8. MIDDLEWARE DE VERIFICAÇÃO CSRF
...
// ========== ROTA PARA RECALCULAR HIERARQUIA (ATUALIZA SÓ PESOS, NÃO MEXE EM COMISSAO_BONUS) ==========
app.post('/api/commission/recalculate-hierarchy', async (req, res) => {
    try {
        ...
        const periodoRef = dataReferencia.substring(0, 7); // 'YYYY-MM'
        console.log(`📅 Recalculando hierarquia para o mês atual: ${dataReferencia} (período ${periodoRef})`);

        // 1. Buscar colaboradores APENAS do período atual
        ...
        console.log(`👥 Total de colaboradores no período ${periodoRef}: ${colaboradores.length}`);

        if (colaboradores.length === 0) {
            return res.status(400).json({ success: false, error: 'Nenhum colaborador encontrado para o período atual' });
        }

        // 2. Buscar métricas existentes para a data
        ...
        // Função que soma apenas os pesos (sem o bônus)
        ...
        // Função que atualiza APENAS os seis campos de peso (comissao_bonus NÃO é alterado)
        const updateMetrics = async (email, soma, tipo) => {
            ...
            console.log(`🔄 ${tipo} - Atualizando pesos para ${email}:`, { somaDiarioAss, somaDiarioGan, somaMensalAss, somaMensalGan });
            ...
            if (existing.rows.length > 0) {
                ...
                console.warn(`⚠️ Registro não encontrado para ${email} em ${dataReferencia}. Nenhuma ação realizada.`);
            }
        };

        // 3. Classificar colaboradores (normalizando grupos)
        const supervisores = colaboradores.filter(c => c.grupo?.toLowerCase() === 'supervisor');
        const coordAdmins = colaboradores.filter(c => c.grupo?.toLowerCase() === 'coordenador');   // apenas coordenadores
        const assessores = colaboradores.filter(c => !supervisores.includes(c) && !coordAdmins.includes(c) && c.grupo?.toLowerCase() !== 'administrativo');
        // Administrativo não é recalculado

        console.log(`👨‍💼 Assessores: ${assessores.length}, Supervisores: ${supervisores.length}, Coordenadores: ${coordAdmins.length}`);

        // 4. Recalcular supervisores (soma dos membros da equipe = apenas assessores da mesma equipe)
        for (const sup of supervisores) {
            ...
            console.log(`📌 Supervisor ${sup.e_mail} (equipe ${sup.equipe}) - membros: ${emailsEquipe.length}`);
            const soma = sumWeights(emailsEquipe);
            await updateMetrics(sup.e_mail, soma, 'Supervisor');
        }

        // 5. Recalcular coordenadores (soma dos supervisores)
        if (supervisores.length > 0) {
            const emailsSupervisores = supervisores.map(s => s.e_mail);
            const somaSupervisores = sumWeights(emailsSupervisores);
            console.log(`🌍 Soma dos supervisores (${emailsSupervisores.length} supervisores)`);
